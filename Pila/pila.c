...
//Determina si una pila piramidal. Una pila de enteros es
//piramidal si cada elemento es menor a su elemento inferior 
//(en el sentido que va desde el tope de la pila hacia el otro extremo).
bool es_pila_piramidal(const pila_t *pila){
    int *act=pila_ver_tope(pila);
    int *act2=pila_ver_tope(pila);
    for (size_t i = pila->cantidad; i > -1; i--){
        act2=pila->datos[i];
        act=pila->datos[i+1];
        if (*act2 < *act) return false;
    }
    return true;   
}